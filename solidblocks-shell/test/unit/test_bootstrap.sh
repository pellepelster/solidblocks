#!/usr/bin/env bash

DIR="$(cd "$(dirname "$0")" ; pwd -P)"

set -eu -o pipefail

source "${DIR}/utils.sh"


ls -lsa "${DIR}/../../build/solidblocks-shell-${VERSION}.zip"

mkdir -p "${TEMP_DIR}/pellepelster/solidblocks/releases/download/${VERSION}/"
cp "${DIR}/../../build/solidblocks-shell-${VERSION}.zip" "${TEMP_DIR}/pellepelster/solidblocks/releases/download/${VERSION}/"

docker run -it -d --rm --name nginx -p 8084:80 -v "${TEMP_DIR}:/usr/share/nginx/html" nginx

sleep 2

SOLIDBLOCKS_BASE_URL="http://localhost:8084" "${DIR}/../../build/documentation/shell_minimal_skeleton_do" bootstrap

function clean() {
  clean_temp_dir
  docker rm -f nginx
}

trap clean HUP INT QUIT TERM EXIT