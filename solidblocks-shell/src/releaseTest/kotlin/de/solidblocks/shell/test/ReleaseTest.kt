  ...
  fun testSnippets(context: SolidblocksTestContext) {
    val version =
        System.getenv("VERSION")
            ?: throw RuntimeException("environment variable VERSION is not set")

    val script =
        context
            .docker(DockerTestImage.DEBIAN_12)
            .script()
            .includes(
                URI.create(
                    "https://github.com/pellepelster/solidblocks/releases/download/$version/blcks-shell-bootstrap-solidblocks-$version.sh",
                ),
            )
