...
public class LogTest {
  ...
  fun testLogMessages(testContext: SolidblocksTestContext) {
    ...
    assertSoftly(result) {
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[  success\\] some success.*"
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[  warning\\] some warning.*"
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[    debug\\] some debug.*"
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[    error\\] some error.*"
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[     info\\] some info.*"
    }
  }
  ...
  fun testLogDie(testContext: SolidblocksTestContext) {
    ...
    assertSoftly(result) {
      ...
      it stderrShouldMatch
          ".*${Calendar.getInstance().get(Calendar.YEAR)}-.* UTC \\[emergency\\] fatal message.*"
    }
  }
}
