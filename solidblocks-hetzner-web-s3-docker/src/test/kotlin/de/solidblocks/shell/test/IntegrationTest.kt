...
public class IntegrationTest {
  ...
  fun testDockerRwUserCanPush() {
    ...
    docker
        .pushImageCmd("$dockerHost/alpine")
        .withTag("latest")
...
fun IntegrationTest.S3Bucket.ownerS3Client(s3Host: String) =
    MinioClient.builder()
...
fun IntegrationTest.S3Bucket.rwS3Client(s3Host: String) =
    MinioClient.builder()
        .endpoint("https://$s3Host")
        .region("garage")
...
fun IntegrationTest.S3Bucket.roS3Client(s3Host: String) =
    MinioClient.builder()
        .endpoint("https://$s3Host")
        .region("garage")
