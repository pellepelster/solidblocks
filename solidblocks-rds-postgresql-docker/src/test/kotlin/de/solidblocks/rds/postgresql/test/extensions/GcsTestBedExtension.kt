...
class GcsTestBedExtension :
    ...
    override fun supportsParameter(parameterContext: ParameterContext, extensionContext: ExtensionContext): Boolean = parameterContext.parameter.type == GcsTestBed::class.java

    @Throws(ParameterResolutionException::class)
    override fun resolveParameter(parameterContext: ParameterContext, context: ExtensionContext): Any = createTestBed(context)

    override fun afterEach(context: ExtensionContext) {
        // testBeds[context.uniqueId]?.destroyTestBed()
    }

    private fun createTestBed(context: ExtensionContext): GcsTestBed = testBeds.getOrPut(context.uniqueId) { GcsTestBed(storageClient) }.also { it.initTestbed() }
