...
class RdsPostgresqlMinioBackupIntegrationTest {
  ...
  companion object {
    const val BACKUP_HOST = "minio"
    const val BUCKET = "database1-backup"
    const val ACCESS_KEY = "database1-user1"
    const val SECRET_KEY = "ccbaa67e-cf26-432f-a11f-0c9e72abccf8"
    const val DATABASE = "database1"
    const val DATABASE_USER = "user1"
    const val DATABASE_PASSWORD = "password1"
    ...
    val s3BackupEnv =
        mapOf(
            ...
            "DB_BACKUP_S3_HOST" to BACKUP_HOST,
            "DB_BACKUP_S3_BUCKET" to BUCKET,
            "DB_BACKUP_S3_ACCESS_KEY" to ACCESS_KEY,
            "DB_BACKUP_S3_SECRET_KEY" to SECRET_KEY,
  ...
  fun testRestoreDatabaseFromEncryptedBackup(rdsTestBed: RdsTestBed) {
    ...
    val postgresContainer1 =
        rdsTestBed.createAndStartPostgresContainer(
            14,
            s3BackupEnv + mapOf("DB_BACKUP_ENCRYPTION_PASSPHRASE" to "yolo123"),
            initWorldReadableTempDir(),
        )
  ...
  private fun createMinioClient(
    ...
    val clientCertificates: HandshakeCertificates =
        HandshakeCertificates.Builder().addTrustedCertificate(certificates[0]).build()
