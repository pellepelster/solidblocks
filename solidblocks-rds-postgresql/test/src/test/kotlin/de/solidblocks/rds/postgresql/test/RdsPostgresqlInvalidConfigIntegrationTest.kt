...
class RdsPostgresqlInvalidConfigIntegrationTest {
    ...
    fun doesNotStartIfNoDataDirIsMounted() {
        ...
        val container =
            GenericContainer("ghcr.io/pellepelster/solidblocks-rds-postgresql:${System.getenv("VERSION") ?: "snapshot"}-rc").apply {
    ...
    fun doesNotStartIfNoBackupMethodSelected(rdsTestBed: RdsTestBed) {
        ...
        with(rdsTestBed.logConsumer) {
            waitForLogLine("[solidblocks-rds-postgresql] either 'DB_BACKUP_S3' or 'DB_BACKUP_LOCAL' has to be activated")
        }
    }
