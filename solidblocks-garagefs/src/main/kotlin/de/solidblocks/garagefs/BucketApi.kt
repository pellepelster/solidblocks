...
class BucketApi(val api: GarageFsApi) {
  ...
  suspend fun createBucket(request: CreateBucketRequest) =
      api.post<BucketInfoResponse>("v2/CreateBucket", request)

  suspend fun updateBucket(id: String, request: UpdateBucketRequest) =
      api.post<BucketInfoResponse>("v2/UpdateBucket?id=$id", request)
}
