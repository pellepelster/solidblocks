...
class AccessKeyApi(val api: GarageFsApi) {
  ...
  suspend fun getKeyInfo(id: String, showSecretKey: Boolean = false) =
      api.get<KeyInfoResponse>("v2/GetKeyInfo?id=$id&showSecretKey=$showSecretKey")

  suspend fun createKey(request: CreateKeyRequest) =
      api.post<KeyInfoResponse>("v2/CreateKey", request)
}
