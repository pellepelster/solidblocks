...
class HetznerApiTest {
    ...
    fun testDnsRRSetFlow() {
        runBlocking {
            ...
            assertSoftly(rrSetsApi.get(name, RRType.A)!!) {
                it.rrset.name shouldBe name
                it.rrset.ttl shouldBe 61
            }

            val recordsUpdateAction = rrSetsApi.updateRecords(name, RRType.A, DnsRRSetsRecordsUpdateRequest(listOf(
                DnsRRSetRecord("127.0.0.2")
            )))
            ...
            assertSoftly(rrSetsApi.get(name, RRType.A)!!) {
                it.rrset.name shouldBe name
                it.rrset.ttl shouldBe 61
                it.rrset.records shouldHaveSize 1
                it.rrset.records[0].value shouldBe "127.0.0.2"
            }
