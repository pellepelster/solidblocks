...
class HetznerApiTest {
    ...
    fun beforeAll() {
        runBlocking {
            ...
            api.servers.list(labelSelectors = mapOf("test" to LabelSelectorValue.Equals("true"))).forEach {
    ...
    fun testServerFlow() {
        runBlocking {
            ...
            val createdServer = api.servers.create(
                ServerCreateRequest(
                    name, "nbg1", "cx22", image = "debian-12", labels = mapOf("test" to "true"), userData = ""
                )
            )
            ...
            api.servers.list(labelSelectors = mapOf("test" to LabelSelectorValue.Equals("true"))).size shouldBe oldServerCountFiltered + 1
    ...
    fun testGetLoadBalancerByName() {
        runBlocking {
            api.loadBalancers.get("application2")!!.name shouldBe "application2"
        }
    }
    ...
    fun testSSHFlow() {
        runBlocking {
            api.sshKeys.list().filter { it.name.startsWith("test") }.forEach {
