...
@Serializable
data class NetworkCreateSubnetRequest(val type: NetworkType, @SerialName("ip_range") val ipRange: String, @SerialName("network_zone") val networkZone: String, @SerialName("vswitch_id") val vswitchId: String)
...
class HetznerNetworksApi(private val api: HetznerApi) :
    ...
    suspend fun waitForAction(id: Long, logCallback: ((String) -> Unit)? = null) = api.waitForAction(id, logCallback, { api.networks.action(it) })

    suspend fun update(id: Long, request: NetworkUpdateRequest) = api.put<NetworkResponseWrapper>("v1/networks/$id", request)

    override suspend fun action(id: Long): ActionResponseWrapper = api.get("v1/networks/actions/$id") ?: throw RuntimeException("failed to get network action")
}
