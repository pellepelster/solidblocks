...
@Serializable
data class ImageResponse(override val id: Long, override val name: String?, val type: ImageType) : HetznerNamedResource<Long>

class HetznerImagesApi(private val api: HetznerApi) : HetznerDeleteResourceApi<Long, ImageResponse> {
    override suspend fun listPaged(page: Int, perPage: Int, filter: Map<String, FilterValue>, labelSelectors: Map<String, LabelSelectorValue>): ImagesListResponseWrapper =
    ...
    suspend fun get(name: String, filter: Map<String, FilterValue> = emptyMap()) = list(mapOf("name" to FilterValue.Equals(name)) + filter).singleOrNull()
}
