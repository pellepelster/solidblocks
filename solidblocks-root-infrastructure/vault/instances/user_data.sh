...
function mount_storage() {
    echo "/dev/sdb /storage/${hostname}   ext4   defaults  0 0" >> /etc/fstab
...
listener "tcp" {
    ...
    tls_cert_file = "/storage/${hostname}/vault/config/certificates/live/${hostname}.${environment}.${root_domain}/fullchain.pem"
    tls_key_file = "/storage/${hostname}/vault/config/certificates/live/${hostname}.${environment}.${root_domain}/privkey.pem"
}
...
api_addr = "https://${hostname}.${environment}.${root_domain}:8200"
cluster_addr = "https://vault.${environment}.${root_domain}:8200"
EOF
}
...
certbot certonly \
    ...
    --domain ${hostname}.${environment}.${root_domain} \
    --domain vault.${environment}.${root_domain}
EOF
}
