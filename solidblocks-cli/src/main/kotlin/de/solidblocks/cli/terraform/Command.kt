...
class BackendsS3Command(val type: TYPE) : CliktCommand("s3") {
    ...
    option("--aws-secret-access-key", envvar = "AWS_SECRET_ACCESS_KEY").help {
        "can also be provided via variable AWS_SECRET_ACCESS_KEY"
    }.required()
    val awsAccessKeyId by option("--aws-access-key-id", envvar = "AWS_ACCESS_KEY_ID").help {
        "can also be provided via variable AWS_ACCESS_KEY_ID"
    }.required()
    val awsRegion by option("--aws-region", envvar = "AWS_REGION").help {
        "can also be provided via variable AWS_REGION"
    }.required()

    val key by option("--key").default("main")
        .help { "Path to the state file inside the S3 Bucket, see also https://developer.hashicorp.com/terraform/language/backend/s3#key" }

    val file by option("--file").path()
        .help { "$type config file to write, if not provided config will be printed to console only" }
