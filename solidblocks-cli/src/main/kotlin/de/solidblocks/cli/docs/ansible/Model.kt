...
data class VariableTableRow(val name: String, val value: String?, val description: String?, val required: Boolean)
...
fun List<VariableTableRow>.toMarkdownTableRow() = this.joinToString("\n") {
    "| ${it.name} | ${it.value ?: "&lt;none&gt;"} | ${it.description ?: "&lt;none&gt;"} | ${it.required}  |"
}

fun emptyRow() = "| &lt;none&gt; | &lt;none&gt; | | &lt;none&gt; |"

data class Role(val name: String, val defaults: List<Variable>, val variables: List<Variable>, val metaData: RoleMetaData)

data class RoleMetaData(val shortDescription: String?, val description: String?, val options: List<Option>)

data class Option(val name: String, val description: String?, val required: Boolean, val default: String?, val type: String?)
