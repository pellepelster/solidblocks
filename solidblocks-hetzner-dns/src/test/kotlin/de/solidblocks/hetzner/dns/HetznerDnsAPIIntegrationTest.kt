...
class HetznerDnsAPIIntegrationTest {
    ...
    fun testFlow() {
        ...
        api.zones(testZone).onSuccess {
            it.zones.forEach {
                api.deleteZone(it.id) shouldBe true
            }
        }.onFailure {
            fail(it)
        }
        ...
        // create zone
        api.createZone(ZoneRequest(testZone)).onSuccess {
            it.zone.name shouldBe testZone
        }.onFailure {
            fail(it)
        }
        ...
        api.updateZone(
            createdZone.id, ZoneRequest(testZone, ttl = 66)
        )
        ...
        api.records(createdZone.id)
            .onSuccess {
                it.records shouldHaveSize 1
            }
            .onFailure {
                fail(it)
            }
        ...
        val createdRecord = api.records(createdZone.id).getOrNull()!!.records.first { it.name == "yolo" }!!

        // check newly created record shows up
        api.records(createdZone.id)
            .onSuccess {
                it.records shouldHaveSize 2
            }
            .onFailure {
                fail(it)
            }
