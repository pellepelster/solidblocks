...
class HetznerDnsApi(
    ...
    private val objectMapper =
        jacksonObjectMapper()
            ...
            .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).registerKotlinModule()
            .registerModules(
                JavaTimeModule()
            )
        ...
        try {
            this.execute().use { response ->
                ...
                if (!response.isSuccessful && response.code != 404) {
                    return Result.failure(RuntimeException("http call failed with '${response.code}' for '${response.request.url}'"))
                }

                if (response.body == null) {
                    return Result.failure(RuntimeException("response body was empty for '${response.request.url}'"))
                }
    ...
    private inline fun <reified T> post(path: String, body: Any): Result<T> {
        val request: Request = Request.Builder()
            .url("${baseApiUrl}/${path}")
            .header("Auth-API-Token", apiKey)
            .post(objectMapper.writeValueAsString(body).toRequestBody("application/json; charset=utf-8".toMediaType()))
            .build()
    ...
    private inline fun <reified T> put(path: String, body: Any): Result<T> {
        val request: Request = Request.Builder()
            .url("${baseApiUrl}/${path}")
            .header("Auth-API-Token", apiKey)
            .put(objectMapper.writeValueAsString(body).toRequestBody("application/json; charset=utf-8".toMediaType()))
            .build()
    ...
    fun updateZone(zoneId: String, request: ZoneRequest): Result<ZoneResponseWrapper> = put("zones/${zoneId}", request)

    fun zones(name: String? = null): Result<ListZonesResponse> = get(
        if (name != null) {
            "zones?name=${name}"
        } else {
            "zones"
