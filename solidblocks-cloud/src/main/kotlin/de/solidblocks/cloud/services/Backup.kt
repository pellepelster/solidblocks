...
object BackupConfigurationFactory {
    ...
    val BACKUP_DATA_VOLUME_SIZE_KEYWORD =
        ...
        ).constraints(VOLUME_SIZE).optional()
...
data class BackupRuntime(val volumeSize: Int?, val fullRetentionDays: Int) {
    companion object {
        fun fromConfig(config: BackupConfig) = BackupRuntime(config.volumeSize, config.fullRetentionDays)
    }

    fun backupVolumeSizeWithDefault(dataVolumeSize: Int) = ByteSize.fromGigabytes(
