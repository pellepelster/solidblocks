    ...
    fun createServiceNode(name: String, location: String, resourceGroup: ResourceGroup, index: Int = 0) {
        val volume = resourceGroup.addResource(
            Volume(
                ...
                location = location,
                labels = ModelConstants.defaultCloudLabels(reference, NodeRole.service)
            )
        )
        ...
        val userData =
            UserData("lib-cloud-init-generated/${NodeRole.service}-cloud-init.sh", staticVariables, ephemeralVariables)
        ...
        resourceGroup.addResource(
            Server(
                ...
                location = location,
                volume = volume,
                labels = defaultServiceLabels(reference, NodeRole.service),
                subnet = null
            )
        )
    }
