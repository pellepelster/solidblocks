...
abstract class BaseNumberKeyword<T>(override val name: String, val constraints: NumberConstraints, override val optional: Boolean, override val default: T?, override val help: KeywordHelp) : SimpleKeyword<T> {
    ...
    open fun parse(yaml: YamlNode): Result<T> {
        ...
        if (constraints.min != null && number < constraints.min) {
            return Error<T>(
                "'$name' may not be smaller then ${constraints.min} at ${yaml.location.logMessage()}",
            )
        }

        if (constraints.max != null && number > constraints.max) {
            return Error<T>(
                "'$name' may not be larger then ${constraints.min} at ${yaml.location.logMessage()}",
            )
        }
...
class NumberKeyword(name: String, constraints: NumberConstraints, help: KeywordHelp) : BaseNumberKeyword<Int>(name, constraints, true, null, help) {
...
class NumberKeywordOptional(name: String, constraints: NumberConstraints, help: KeywordHelp) : BaseNumberKeyword<Int?>(name, constraints, true, null, help) {
...
class NumberKeywordOptionalWithDefault(name: String, constraints: NumberConstraints, help: KeywordHelp, default: Int) : BaseNumberKeyword<Int>(name, constraints, true, default, help) {
