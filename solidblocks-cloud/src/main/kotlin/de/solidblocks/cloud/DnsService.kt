...
class DnsService() {
    ...
    val dnsServers =
        listOf("208.67.222.222", "208.67.220.220", "8.8.8.8", "8.8.4.4", "1.1.1.1", "1.0.0.1", "9.9.9.9", "9.9.9.10")
    ...
    fun tryResolveARecords(name: String): List<DnsRecord> = dnsServers.flatMap { resolver ->
        ...
        if (records.isEmpty()) {
            ...
            records.map {
                DnsRecord(resolver, it.name.toString(), listOf(it.address.hostAddress))
            }
        }
    }
    ...
    private inline fun <reified T : Record> getRecords(resolver: Resolver, name: String, type: Int): List<T> {
        ...
        return resolver.send(newQuery(queryRecord)).getSectionRRsets(Section.ANSWER).flatMap { it.rrs() }.filterIsInstance<T>()
    }
