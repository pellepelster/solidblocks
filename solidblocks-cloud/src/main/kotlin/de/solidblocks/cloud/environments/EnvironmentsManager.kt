...
class EnvironmentsManager(
    ...
    fun createEnvironmentForDefaultCloud(reference: CloudReference,
                                         name: String,
                                         email: String,
                                         password: String,
                                         ...
                                         hetznerDnsApiToken: String): CreationResult<EnvironmentEntity> {
        val cloud = cloudsManager.newEnvironmentsDefaultCloud(email)
                ?: return CreationResult.error(ErrorCodes.CLOUD.NOT_FOUND)
        return create(CloudReference(cloud.name), name, email, password, githubReadOnlyToken, hetznerCloudApiTokenReadOnly, hetznerCloudApiTokenReadWrite, hetznerDnsApiToken)
    }
    ...
    fun create(
        ...
        if (environmentsRepository.hasEnvironment(reference.toEnvironment(name))) {
            ...
            return CreationResult.error(ErrorCodes.ENVIRONMENT.DUPLICATE)
        }
        ...
        return CreationResult(environment)
    }

    fun hasCloud(reference: EnvironmentReference) = environmentsRepository.hasEnvironment(reference)

    fun verifyReference1(reference: EnvironmentReference): Boolean {
        if (!cloudsManager.hasCloud(reference)) {
            return false
        }
    ...
    fun validate(request: EnvironmentCreateRequest): ValidationResult {
        ...
        val cloud = cloudsManager.newEnvironmentsDefaultCloud(request.email)
                ?: return ValidationResult.error(ErrorCodes.ENVIRONMENT.CLOUD_NOT_FOUND)
