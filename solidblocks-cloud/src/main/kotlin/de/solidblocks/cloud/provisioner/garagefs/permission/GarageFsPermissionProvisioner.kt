...
class GarageFsPermissionProvisioner :
    ...
    suspend fun lookupInternal(lookup: GarageFsPermissionLookup, context: CloudProvisionerContext): Result<GarageFsPermissionRuntime?> = context.withApiClients(lookup.server, lookup.adminToken) { apis ->
        when (apis) {
    ...
    override suspend fun lookup(lookup: GarageFsPermissionLookup, context: CloudProvisionerContext) = when (val result = lookupInternal(lookup, context)) {
        is Error<GarageFsPermissionRuntime?> -> null
        is Success<GarageFsPermissionRuntime?> -> result.data
    }

    override suspend fun apply(resource: GarageFsPermission, context: CloudProvisionerContext, log: LogContext): Result<GarageFsPermissionRuntime> {
        val bucket =
    ...
    override suspend fun diff(resource: GarageFsPermission, context: CloudProvisionerContext) = when (val result = lookupInternal(resource.asLookup(), context)) {
