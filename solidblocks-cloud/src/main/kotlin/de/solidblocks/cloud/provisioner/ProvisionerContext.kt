...
data class ProvisionerContext(
    ...
    fun <RuntimeType, ResourceLookupType : InfrastructureResourceLookup<RuntimeType>> lookup(lookup: ResourceLookupType): RuntimeType? = registry.lookup(lookup, this)
    ...
    fun validateDnsZone(domain: String): Result<String> {
        ...
        return if (domainLookup == null) {
            Error("no zone found for root domain '${domain}', please make sure that the zone can be managed by the configured cloud provider")
        } else {
            Success(domain)
        }
    }
