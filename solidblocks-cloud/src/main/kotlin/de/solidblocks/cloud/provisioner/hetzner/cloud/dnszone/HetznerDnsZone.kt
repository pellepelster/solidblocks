package de.solidblocks.cloud.provisioner.hetzner.cloud.dnszone

import de.solidblocks.cloud.api.resources.BaseLabeledInfrastructureResource

class HetznerDnsZone(name: String, labels: Map<String, String> = emptyMap()) : BaseLabeledInfrastructureResource<HetznerDnsZoneRuntime>(name, emptySet(), labels) {
    override fun asLookup() = HetznerDnsZoneLookup(name)

    override val lookupType = HetznerDnsZoneLookup::class

    override fun logText() = "DNS zone '$name'"
}
