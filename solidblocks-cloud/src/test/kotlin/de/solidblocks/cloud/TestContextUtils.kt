...
data class HetznerTestContext(val provisioner: Provisioner, val serverProvisioner: HetznerServerProvisioner, val context: ProvisionerContext) {
    ...
    companion object {
        fun create(hcloudToken: String): HetznerTestContext {
            ...
            val registry = ProvisionersRegistry(
                listOf(
                    UserDataLookupProvider(), sshProvisioner, volumeProvisioner, serverProvisioner, networkProvisioner, subnetProvisioner
                ),
                listOf(
                    sshProvisioner, volumeProvisioner, serverProvisioner, networkProvisioner, subnetProvisioner
                ),
            )
            val provisioner = Provisioner(registry)

            return HetznerTestContext(
                provisioner, serverProvisioner, TEST_PROVISIONER_CONTEXT.copy(
                    registry = registry
                )
            )
        }
