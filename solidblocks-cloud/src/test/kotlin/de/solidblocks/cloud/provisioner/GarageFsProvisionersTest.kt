...
class GarageFsProvisionersTest {
    ...
    val garageFsContainer = GenericContainer(
        ImageFromDockerfile("localhost/testcontainers/" + Base58.randomString(16).lowercase(getDefault()), false).withFileFromClasspath("Dockerfile", "garagefs/Dockerfile")
            .withFileFromClasspath("supervisord.conf", "garagefs/supervisord.conf").withFileFromClasspath("garage.toml", "garagefs/garage.toml").withFileFromClasspath("authorized_keys", "test_ed25519.key.pub"),
    ).also {
    ...
    val key = SSHKeyUtils.tryLoadKey(GarageFsProvisionersTest::class.java.getResource("/test_ed25519.key").readText())
    ...
    fun testFlow() {
        ...
        coEvery { secretProvisioner.lookup(any(), any()) } returns PassSecretRuntime(
            "admin_token",
        ...
        val context = TEST_PROVISIONER_CONTEXT.copy(
        ...
        runBlocking {
            ...
            bucketProvisioner.apply(bucket, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsBucketRuntime>>().data.name shouldBe bucket.name
            bucketProvisioner.apply(bucket, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsBucketRuntime>>().data.name shouldBe bucket.name

            accessKeyProvisioner.apply(accessKey, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsAccessKeyRuntime>>().data.name shouldBe accessKey.name
            accessKeyProvisioner.apply(accessKey, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsAccessKeyRuntime>>().data.name shouldBe accessKey.name
            ...
            val bucketWithWebsiteAccess = GarageFsBucket(bucketName, server, adminToken, websiteAccess = true)
            ...
            bucketProvisioner.apply(
                bucketWithWebsiteAccess,
                context,
                TEST_LOG_CONTEXT,
            ).shouldBeTypeOf<Success<GarageFsBucketRuntime>>().data.name shouldBe bucket.name
            assertSoftly(bucketProvisioner.diff(bucketWithWebsiteAccess, context)) {
                it.status shouldBe ResourceDiffStatus.up_to_date
            }

            val bucketWithWebsiteAccessDomains = GarageFsBucket(
                bucketName,
                server,
                adminToken,
                websiteAccess = true,
            ...
            permissionProvisioner.apply(allPermission, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsBucketRuntime>>().data shouldNotBe null
            ...
            permissionProvisioner.apply(
                ...
                context,
                TEST_LOG_CONTEXT,
            ).shouldBeTypeOf<Success<GarageFsBucketRuntime>>().data shouldNotBe null
            assertSoftly(permissionProvisioner.lookup(allPermission.asLookup(), context)!!) {
                it.name shouldBe "${bucket.name}.${accessKey.name}"
                it.owner shouldBe false
                it.read shouldBe true
            ...
            permissionProvisioner.apply(
                ...
                context,
                TEST_LOG_CONTEXT,
            ).shouldBeTypeOf<Success<GarageFsPermissionRuntime>>().data shouldNotBe null
            ...
            permissionProvisioner.apply(allPermission, context, TEST_LOG_CONTEXT).shouldBeTypeOf<Success<GarageFsPermissionRuntime>>().data shouldNotBe null
