...
public enum class LogType { solidblocks, stdout, stderr }

fun logInfo(logType: LogType, message: String) = println("[${logType}] ${message}")

fun logError(logType: LogType, message: String) = println("[${logType}] ${color(message, COLORS.RED)}")

fun logSuccess(logType: LogType, message: String) = println("[${logType}] ${color(message, COLORS.GREEN)}")

fun logWarning(logType: LogType, message: String) = println("[${logType}] ${color(message, COLORS.YELLOW)}")
...
public fun List<String>.indentWithYamlObjectMarker() = this.withIndex()
    .map {
        if (it.index == 0) {
            "- ${it.value}"
        } else {
...
fun color(text: String, color: COLORS) = "${escapeCode(color.color)}${text}${escapeCode(RESETS.COLOR.reset)}"
