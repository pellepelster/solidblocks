...
public class CommandSnippets {
    ...
    fun longRunningCommandSnippet(testContext: SolidblocksTestContext) {
        ...
        val command = tempDir.file("long-running-script.sh")
            .content(longRunningCommand).executable().create()

        val result = testContext.local().command(command).assert {
            it.waitForOutput(".*something has happened.*")
        }.assert {
            it.waitForOutput(".*something else has happened.*")
        }.runResult()
    ...
    fun respondToCommandSnippet(testContext: SolidblocksTestContext) {
        ...
        val tempDir = testContext.createTempDir()
        val command = tempDir.file("respond-to-command.sh")
            .content(respondToCommand).executable().create()

        val result = testContext.local().command(command).assert {
            it.waitForOutput(".*please enter name.*") {
                "Steve McQueen"
            }
        }.assert {
            it.waitForOutput(".*name was entered.*")
        }.runResult()
    ...
    fun dockerCommandSnippet(testContext: SolidblocksTestContext) {
        val result = testContext.docker(DockerTestImage.UBUNTU_22)
            .command("whoami").runResult()
