...
public class CommandTest {
    ...
    private fun getCommandPath(path: String) = Path(
    ...
    fun cleanDockerImages() {
        docker
            ...
            .exec()
            .filter { container ->
                TestConstants.dockerTestImageLabels.all { container.labels[it.key] == it.value }
            }
            .forEach {
    ...
    @ParameterizedTest
    @FieldSource("contexts")
    fun testWaitForOutputRunResultNegative(context: CommandTestContext<CommandBuilder, ScriptBuilder>) {
        val exception =
    ...
    @ParameterizedTest
    @FieldSource("contexts")
    fun testWaitForOutputNotMatchedRunResult(context: CommandTestContext<CommandBuilder, ScriptBuilder>) {
        runBlocking {
            val run = context.command(getCommandPath("command-waitfor.sh")).run()
