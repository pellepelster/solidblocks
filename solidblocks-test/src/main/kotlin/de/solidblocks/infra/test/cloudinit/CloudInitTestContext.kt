...
class CloudInitTestContext(val host: String, val keyPair: KeyPair, val username: String = "root", val port: Int = 22, testId: String? = null) : TestContext(testId) {
    companion object {
    ...
    fun result(): CloudInitResultWrapper? = ssh.command("cat /var/lib/cloud/data/status.json").stdout.let {
        try {
