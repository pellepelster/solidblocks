...
class SolidblocksTestContext(testId: String) : TestContext(testId) {
    ...
    fun terraform(dir: Path, version: String? = null) = terraformTestContext(dir, version, testId).also { testContexts.add(it) }

    fun terraform(dir: String, version: String? = null) = terraformTestContext(Path.of(dir), version, testId).also { testContexts.add(it) }

    fun ssh(host: String, privateKey: String, username: String = "root", port: Int = 22) = sshTestContext(host, SSHKeyUtils.loadKey(privateKey), username, port, testId).also {
        testContexts.add(it)
    }

    fun ssh(host: String, keyPair: KeyPair, username: String = "root", port: Int = 22) = sshTestContext(host, keyPair, username, port, testId).also { testContexts.add(it) }

    fun host(host: String) = hostTestContext(host).also { testContexts.add(it) }

    fun hetzner(hcloudToken: String) = hetznerTestContext(hcloudToken, testId).also { testContexts.add(it) }

    fun cloudInit(host: String, privateKey: String, username: String = "root", port: Int = 22) = cloudInitTestContext(host, privateKey, username, port, testId).also { testContexts.add(it) }
