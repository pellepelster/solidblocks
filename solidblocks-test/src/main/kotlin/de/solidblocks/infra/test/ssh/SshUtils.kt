...
object SshUtils {
    ...
    ).mapNotNull { it.invoke() }.firstOrNull() ?: throw RuntimeException("could load private key")

    fun loadRSAKeyPairFromPem(pemKey: String) = try {
    ...
    fun loadECDSAKeyPairFromPem(pemKey: String) = try {
        val pemParser = PEMParser(StringReader(pemKey))
        val pemObject = pemParser.readObject()
        pemParser.close()
    ...
    fun loadEd25519KeyPairFromPem(key: String) = try {
        ...
        val privateKey: PrivateKey = when (pemObject) {
        ...
        val bcPrivateKey = Ed25519PrivateKeyParameters(
            privateKey.encoded.takeLast(32).toByteArray(), 0
        )
    ...
    fun loadEd25519KeyPairFromOpenSSH(key: String) = try {
    ...
    fun loadRSAKeyPairFromOpenSSH(key: String) = try {
        val pemObject = PemReader(StringReader(key)).readPemObject()
        val privateKey = OpenSSHPrivateKeyUtil.parsePrivateKeyBlob(pemObject.content)
        privateKey.toKeyPair()
    } catch (e: Exception) {
    ...
    private fun convertToKeyPair(
        ...
        val publicKeyInfo: SubjectPublicKeyInfo = SubjectPublicKeyInfoFactory.createSubjectPublicKeyInfo(publicKeyParam)
