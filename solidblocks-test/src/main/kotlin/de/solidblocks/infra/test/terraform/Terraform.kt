...
class Terraform(val dir: Path, versionOverride: String? = null, val environment: Map<String, String> = emptyMap()) {
    ...
    fun addEnvironmentVariable(name: String, value: Any) {
        environmentVariables[name] = value.toString()
    }
    ...
    private fun checksumMatches(checksumFile: File, file: File) = file.exists() && extractChecksum(checksumFile, file.name) == calculateSHA256(file)
